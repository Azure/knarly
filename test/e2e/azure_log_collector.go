/*
via: https://github.com/kubernetes-sigs/cluster-api-provider-azure/blob/master/test/e2e/azure_logcollector.go
...
func collectLogsFromNode(ctx context.Context, managementClusterClient client.Client, cluster *clusterv1.Cluster, hostname string, isWindows bool, outputPath string) error {
	...
	if isWindows {
		...
		var errs []error
		errs = append(errs, kinderrors.AggregateConcurrent(windowsInfo(execToPathFn)))
		errs = append(errs, kinderrors.AggregateConcurrent(windowsK8sLogs(execToPathFn)))
		errs = append(errs, kinderrors.AggregateConcurrent(windowsNetworkLogs(execToPathFn)))
		return kinderrors.NewAggregate(errs)
	}
...
func getAzureMachinePool(ctx context.Context, managementClusterClient client.Client, mp *clusterv1exp.MachinePool) (*infrav1exp.AzureMachinePool, error) {
	key := client.ObjectKey{
...
func getAzureManagedMachinePool(ctx context.Context, managementClusterClient client.Client, mp *clusterv1exp.MachinePool) (*infrav1exp.AzureManagedMachinePool, error) {
	key := client.ObjectKey{
		Namespace: mp.Spec.Template.Spec.InfrastructureRef.Namespace,
		Name:      mp.Spec.Template.Spec.InfrastructureRef.Name,
	}
